 ...
 * @group snapshot
 */
final class CommunicationSnapshotTest extends TestCase
{
    public function testCommunicationApiSnapshots(): void
    {
        $snapshotConfig = SnapshotConfig::fromEnvironment(
            __DIR__ . '/Snapshots/communication',
            'communication'
        );
        ...
        if (!filter_var(getenv('SAASUS_E2E') ?: 'false', FILTER_VALIDATE_BOOLEAN)) {
            $this->markTestSkipped('Set SAASUS_E2E=true to run Communication snapshot tests.');
        }
        ...
        $client = CommunicationApiTest::createCommunicationClient($config);
        $stories = array_values(array_filter(
            CommunicationApiTest::communicationStories($client),
            static fn ($story): bool => $snapshotConfig->matchesStory($story->name)
        ));
        self::assertNotSame([], $stories, 'No Communication stories matched the snapshot filter.');

        $engine = new E2EEngine($client, CommunicationApiTest::methods(), $config);
        ...
        $snapshots = $manager->createSnapshots($stories, $results, 'communication');
