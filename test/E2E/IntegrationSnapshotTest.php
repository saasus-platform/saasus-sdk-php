 ...
 * @group snapshot
 */
final class IntegrationSnapshotTest extends TestCase
{
    public function testIntegrationApiSnapshots(): void
    {
        $snapshotConfig = SnapshotConfig::fromEnvironment(
            __DIR__ . '/Snapshots/integration',
            'integration'
        );
        ...
        if (!filter_var(getenv('SAASUS_E2E') ?: 'false', FILTER_VALIDATE_BOOLEAN)) {
            $this->markTestSkipped('Set SAASUS_E2E=true to run Integration snapshot tests.');
        }
        ...
        $client = IntegrationApiTest::createIntegrationClient($config);
        $stories = array_values(array_filter(
            IntegrationApiTest::integrationStories($client),
            static fn($story): bool => $snapshotConfig->matchesStory($story->name)
        ));
        self::assertNotSame([], $stories, 'No Integration stories matched the snapshot filter.');

        $engine = new E2EEngine($client, IntegrationApiTest::methods(), $config);
        ...
        $snapshots = $manager->createSnapshots($stories, $results, 'integration');
