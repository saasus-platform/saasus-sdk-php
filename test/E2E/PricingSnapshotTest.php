...
{
    public function testPricingApiSnapshots(): void
    {
        $snapshotConfig = SnapshotConfig::fromEnvironment(
            __DIR__ . '/Snapshots/pricing',
            'pricing'
        );
        ...
        if (!filter_var(getenv('SAASUS_E2E') ?: 'false', FILTER_VALIDATE_BOOLEAN)) {
            $this->markTestSkipped('Set SAASUS_E2E=true to run Pricing snapshot tests.');
        }
        ...
        $client = PricingApiTest::createPricingClient($config);
        $stories = array_values(array_filter(
            PricingApiTest::pricingStories($client),
            static fn ($story): bool => $snapshotConfig->matchesStory($story->name)
        ));
        self::assertNotSame([], $stories, 'No Pricing stories matched the snapshot filter.');

        $engine = new E2EEngine($client, PricingApiTest::methods(), $config);
        ...
        $snapshots = $manager->createSnapshots($stories, $results, 'pricing');
